...
namespace alpaca
{
    ...
    DLLFUNC_C int BrokerLogin(char* User, char* Pwd, char* Type, char* Account)
    {
        ...
        alpacaMD = std::make_unique<AlpacaMarketData>(client->headers(), client->logger(), config.paidPlan);

        if (!config.polygonApiKey.empty()) {
            polygon = std::make_unique<Polygon>(config.polygonApiKey, client->logger(), config.paidPlan);
        } 
        ...
        if (!config.dataSource) {
            ...
            if (!wsClient->login(s_logger, User, Pwd, config.paidPlan ? ALPACA_PRO_DATA_WS_URL : ALPACA_BASIC_DATA_WS_URL)) {
    ...
    DLLFUNC_C int BrokerAsset(char* Asset, double* pPrice, double* pSpread, double* pVolume, double* pPip, double* pPipCost, double* pLotAmount, double* pMarginCost, double* pRollLong, double* pRollShort)
    {
        ...
        if (s_priceType == 2) {
            auto trade = wsClient->getLastTrade(Asset);
            if (trade) {
                if (pPrice) {
                    *pPrice = trade->price;
                }
    ...
    DLLFUNC_C int BrokerHistory2(char* Asset, DATE tStart, DATE tEnd, int nTickMinutes, int nTicks, T6* ticks)
    {
        ...
        auto populateTicks = [&barsDownloaded, nTickMinutes, &end, &ticks](std::vector<Bar>& bars) {
        ...
        if (config.dataSource == 0 && !config.paidPlan) {
    ...
    DLLFUNC_C double BrokerCommand(int Command, DWORD dwParameter)
    {
        ...
        case 2000:
            if ((int)dwParameter != 0) {
                ...
                if (!alpacaMD) {
                    alpacaMD = std::move(std::make_unique<AlpacaMarketData>(client->headers(), client->logger(), false));
                }
